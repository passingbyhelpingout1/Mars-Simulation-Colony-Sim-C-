...
static void showStatus(const GameState& s) {
  std::cout << "\n=== Colony Status ===\n";
  std::cout << "Time: Sol " << s.sol() << ", Hour " << s.hourOfSol()
            << " (T+" << s.hour << "h)\n";
  std::cout << "Colonists: " << s.colonists << "\n";
  std::cout << "Installations: " << s.solarPanels << " Solar, "
            << s.batteries << " Batteries, "
            << s.labs << " Lab(s)\n";
  std::cout << std::fixed << std::setprecision(2);
  std::cout << "Battery: " << s.res.powerStored << " / "
            << s.res.powerCapKWh << " kWh\n";
  std::cout << "Weather: "
            << (s.weather.dustStorm
                ? "Dust storm (" + std::to_string(s.weather.dustStormHours) + "h left)"
                : "Clear")
            << "\n";

  // Quick estimates for the current hour
  ...
  std::cout << "Now (estimates): Gen " << solarKW << " kW, Critical "
            << critKW << " kW, Non-crit potential " << nonKW << " kW\n";

  if (s.lastPower.criticalDemand > 0.0) {
    std::cout << "Last hour: Gen " << s.lastPower.producers
              << " kW, Critical " << s.lastPower.criticalDemand
              << " kW, Non-crit run " << (100.0 * s.lastPower.nonCriticalEff)
...
int run() {
  GameState s;
  initDefaultGame(s, 42u); // deterministic seed by default
